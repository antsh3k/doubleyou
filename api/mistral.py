...
@router.post("/api/py/mistral")
async def mistral_endpoint(
    message: Message, conversation: Conversation = Depends(get_conversation)
):
    ...
    chat_response = client.chat.complete(model=model, messages=conversation.messages)
...
Analyze the input image and output the information using the following JSON schema:
...
If you cannot extract certain information, use "Unknown" for string fields, an empty list for array fields, and false for boolean fields.
"""
